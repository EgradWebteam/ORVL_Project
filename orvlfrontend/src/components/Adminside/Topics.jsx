...
const Topics = () => {
  ...
  const handleEdit = async (topic) => {
    ...
    try {
      ...
      setCurrentTopic(fetchedTopic);
      setSelectedExam(fetchedTopic.exam_id);
      setSelectedSubject(fetchedTopic.subject_id);
      setEditModal(true);
    } catch (error) {
  ...
  const handleUpdate = async (event) => {
    event.preventDefault();
    if (!currentTopic || !currentTopic.topic_name) {
    ...
    try {
      await axios.put(`http://localhost:8000/TopicCreation/topics/update-topic-name/${currentTopic.topic_id}`, updatedData);
                    ...
                    {topics.map((topic, index) => (
                      <div key={index} className="topic-entry">
                        <input
                          ...
                          onChange={(event) => handleTopicChange(index, { ...topic, topic_name: event.target.value })}
          ...
          <div className='modal'>
            ...
            <div className='content_m'>
              ...
              <form onSubmit={handleUpdate}>
                ...
                <div className='div1'>
                  <h3 className='headertv'>Enter Topic Name:</h3>
                  <input
                    type="text"
                    value={currentTopic?.topic_name || ''}
                    onChange={(event) => setCurrentTopic({ ...currentTopic, topic_name: event.target.value })}
                    placeholder="Enter topic name"
                    className='input'
                  />
                </div>
